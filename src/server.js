#!/usr/bin/env node
// mistral-mcp · stdio MCP server exposing foldkit as Claude-callable tools + resources
...
const server = new Server(
  { name: 'mistral-mcp', version: '1.0.0' },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  try {
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `mistral-mcp error: ${err.message}` }]
    };
  }
});
